...
app.get('/.well-known/web-app-origin-association', (req, res) => {
  ...
  res.sendFile(__dirname + '/static/.well-known/web-app-origin-association.json');
});
...
function getGameRoomIDs() {
  return [...io.sockets.adapter.rooms.keys()].filter(roomId => roomId.startsWith(gameRoomPrefix));
}
...
function join(socket, roomId=null) {
  ...
  if (!roomData[final]) {
    ...
    for (let i = 0; i < obstaclesPerRoom; i++) {
      const x = Math.floor(Math.random() * (obstaclesXYBounds[1] - obstaclesXYBounds[0] + 1) + obstaclesXYBounds[0]);
...
function start(roomId) {
  ...
  // choose role
  let killers = [];
  const numKillers = io.sockets.adapter.rooms.get(roomId).size < Math.floor(maxPlayersInRoom / maxKillersInRoom - 0.5)? minKillersInRoom : maxKillersInRoom;
  for (let i = 0; i < numKillers; i++) {
    let r = null;
    while (true) {
      r = Math.floor(Math.random() * (io.sockets.adapter.rooms.get(roomId).size));
      if (!killers.includes(r))
        break;
    }
    killers.push(r);
  }
...
function logRooms() {
  ...
  console.log('Rooms:', (ids.length == 0? 'none' : ids.map(room => `${room}(${io.sockets.adapter.rooms.get(room).size})`).join(',')));
}
...
io.on('connection', socket => {
  ...
  socket.on('nick', nick => {
    nick = nick.trim().substr(0, maxNickLength).replace(/['"\/\\<>\u200b\u2800]/gi, '').toUpperCase();
    if (nick)
  ...
  socket.on('move', data => {
    if (typeof data == 'object') {
      if (typeof data.x == 'number')
        players[socket.id].move.x = data.x;

      if (players[socket.id].move.x > 1)
        players[socket.id].move.x = 1;
      else if (players[socket.id].move.x < -1)
        players[socket.id].move.x = -1;

      players[socket.id].flipX = players[socket.id].move.x > 0? false : (players[socket.id].move.x < 0? true : players[socket.id].flipX);
    }
  });

  socket.on('jump', () => {
    if ((!players[socket.id].frozen) && players[socket.id].alive && players[socket.id].y >= groundY) {
  ...
  socket.on('ability', (ability, callback) => {
    ...
    switch (ability) {
      case 'freeze':
        ...
        for (const id of io.sockets.adapter.rooms.get(players[socket.id].roomId)) {
          ...
          if (players[socket.id].role != players[id].role) {
            ...
            // save frozen time
            players[id].frozenTime = players[socket.id].abilities[ability].effect;
          }
        }
        break;
      case 'heal':
        ...
        for (const id of io.sockets.adapter.rooms.get(players[socket.id].roomId)) {
          ...
          if (
            id != socket.id && (!players[id].alive) && players[id].role == 'runner' &&
            players[id].x >= players[socket.id].x - abilities.runner.heal.hitbox.dx &&
            players[id].x <= players[socket.id].x + abilities.runner.heal.hitbox.dx &&
            players[id].y >= players[socket.id].y - abilities.runner.heal.hitbox.dy &&
            players[id].y <= players[socket.id].y + abilities.runner.heal.hitbox.dy
          ) {
  ...
  socket.on('disconnect', reason => {
    ...
    if (roomData[players[socket.id].roomId].starting && io.sockets.adapter.rooms.get(players[socket.id].roomId).size < minPlayersInRoom) {
...
setInterval(() => {
  for (let roomId of getGameRoomIDs()) {
      ...
      for (let player_id of roomPlayerIDs) {
        ...
        if (player.frozen) {
          ...
          if (player.frozenTime <= 0)
            player.frozen = false;
        } else if (player.alive && ((!('heal' in player.abilities)) || player.abilities.heal?.effect <= 0)) {
          // move player
          player.x += player.move.x * (('sprint' in player.abilities && player.abilities.sprint.effect > 0)? (abilities[player.role].sprint?.speed || 1) : 1);
        }

        // gravity
        player.y += player.move.y;
        if (player.y < groundY)
          player.move.y += gravity;
        else
          player.move.y = 0;
  
        // if is a killer not released yet
        if (player.role == 'killer' && roomData[roomId].releaseKillersTime > 0) {
        ...
        // spice ability kills
        const doingSpice = player.abilities.spice && player.abilities.spice.effect > 0;
        ...
        if (doingSpice || doingSpice2) {
          ...
          for (const id of roomPlayerIDs) {
            ...
            if (
              doingSpice && player_id != id && players[id].role != 'killer' && players[id].alive &&
            ...
            if (
              doingSpice2 && player_id != id && players[id].role != 'killer' && players[id].alive &&
    ...
    if ((!roomData[roomId].started) && roomData[roomId].starting && roomData[roomId].startingTime <= 0) {
      start(roomId);
    }
    ...
    if (roomData[roomId].started && (!roomData[roomId].ended) && allExceptKillersFinished) {
    ...
    else if (roomData[roomId].started && (!roomData[roomId].ended) && allExceptKillersDead && roomData[roomId].releaseKillersTime < releaseKillersMaxTime - 300) {
      roomData[roomId].ended = true;
